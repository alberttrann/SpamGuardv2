...
# I've downloaded the email_spam.csv file from Kaggle to the backend/data directory.
EVALUATION_CSV_PATH = "email_spam.csv"
...
def evaluate_on_local_csv():
    """
    Main function to evaluate the SpamGuard Hybrid model on the local email_spam.csv dataset.
    """
    dataset_name = os.path.basename(EVALUATION_CSV_PATH)
    print("\n" + "="*80)
    print(f"  STARTING EVALUATION ON LOCAL CSV: {dataset_name}")
    print("="*80)
    ...
    # --- 2. Load and Prepare the Local CSV Dataset ---
    print(f"\n[Step 3/4] Loading and preparing local dataset '{dataset_name}'...")
    if not os.path.exists(EVALUATION_CSV_PATH):
        print(f"❌ ERROR: Evaluation file not found at '{EVALUATION_CSV_PATH}'."); return
    
    try:
        df_eval = pd.read_csv(EVALUATION_CSV_PATH)
        # Drop rows where 'text' or 'type' is missing
        df_eval.dropna(subset=['text', 'type'], inplace=True)
        
        # Extract and map the labels: 'not spam' -> 'ham'
        true_labels = df_eval['type'].str.lower().replace('not spam', 'ham').tolist()
        eval_messages = df_eval['text'].astype(str).tolist()
        
        print(f"✅ Loaded and preprocessed {len(eval_messages)} messages from the CSV file.")
    except Exception as e:
        print(f"❌ ERROR: Failed to read or process the CSV file. Details: {e}"); return
    ...
    for message in tqdm(eval_messages, desc=f"Classifying {dataset_name}"):
    ...
    print("\n" + "="*80)
    print(f"  FINAL REPORT: SpamGuard Hybrid on '{dataset_name}'")
    print("="*80)
    ...
    print(f"Total Prediction Time: {total_time_s:.2f} seconds")
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='viridis', xticklabels=all_known_labels, yticklabels=all_known_labels)
    plt.title(f'SpamGuard Hybrid Performance\non {dataset_name}')
    plt.ylabel('True Label'); plt.xlabel('Predicted Label'); plt.show()


if __name__ == "__main__":
    evaluate_on_local_csv()