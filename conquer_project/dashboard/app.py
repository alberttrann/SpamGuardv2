...
if 'generation_type' not in st.session_state:
    st.session_state.generation_type = None # Will store 'spam', 'ham', or 'random'
...
message_input = st.text_area("Enter a message to analyze:", height=100)
...
# SECTION 3: LLM DATA DISTILLATION (Updated)
...
if use_llm:
    provider = st.radio("Choose LLM Provider", ["Ollama (Local)", "OpenRouter (Cloud)"], horizontal=True)
    
    if provider == "Ollama (Local)":
        model_id = st.text_input("Ollama Model ID", "llama3")
        api_key = ""
    else: # OpenRouter
        model_id = st.text_input("OpenRouter Model ID", "mistralai/mistral-7b-instruct")
    ...
    if gen_col1.button("🤖 Generate Random (Spam or Ham)", use_container_width=True, disabled=st.session_state.generating):
        st.session_state.generating = True
        st.session_state.generation_type = None # None means random
        st.rerun()

    if gen_col2.button("🚨 Generate SPAM only", use_container_width=True, disabled=st.session_state.generating):
        st.session_state.generating = True
        st.session_state.generation_type = "spam"
        st.rerun()
    
    if gen_col3.button("✅ Generate HAM only", use_container_width=True, disabled=st.session_state.generating):
        st.session_state.generating = True
        st.session_state.generation_type = "ham"
        st.rerun()

    if st.session_state.generating:
        st.warning("LLM is now generating data. Press 'Stop Generating' to halt the process.")
        ...
        while st.session_state.generating:
            ...
            try:
                ...
                with requests.post(f"{API_BASE_URL}/generate_data", json=payload, stream=True, timeout=60) as r:
                    r.raise_for_status()
                    for chunk in r.iter_content(chunk_size=None, decode_unicode=True):
                        # Check again inside the loop in case the stop button was pressed
                        if not st.session_state.generating:
                            status_box.info("Generation stopped by user.")
                            break
                        
                        if chunk.startswith("data:"):
                            status_message = chunk.split("data:")[1].strip()
                            status_box.write(f"🤖 LLM Status: {status_message}")
                            time.sleep(1) # Pause to make it readable
                
                # If the loop finishes without being stopped, pause before next run
                if st.session_state.generating:
                    time.sleep(2)
