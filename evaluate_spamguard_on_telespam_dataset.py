...
HF_DATASET_ID = "thehamkercat/telegram-spam-ham"
...
# --- Global Components & Helper Functions ---
...
def evaluate_on_huggingface():
    """
    Main function to evaluate the final SpamGuard Hybrid model on a public HF dataset.
    """
    print(f"\n" + "="*80); print(f"  STARTING EVALUATION ON HUGGING FACE DATASET: {HF_DATASET_ID}"); print("="*80)

    # --- 1. Load SpamGuard Components ---
    print(f"\n[Step 1/3] Loading local SpamGuard model '{MODEL_ID_TO_TEST}'...");
    ...
    try: nb_pipeline = joblib.load(pipeline_path); le = joblib.load(encoder_path)
    except FileNotFoundError: print(f"❌ ERROR: Model files for ID '{MODEL_ID_TO_TEST}' not found."); return
    print("\n[Step 2/3] Building FAISS index from local training data...")
    ...
    print(f"\n[Step 3/3] Downloading and preparing Hugging Face dataset '{HF_DATASET_ID}'...")
    try:
        dataset = load_dataset(HF_DATASET_ID, split='train')
        dataset = dataset.filter(lambda x: x['text_type'] is not None and x['text'] is not None and x['text'].strip() != "")
        
        if HF_DATASET_ID == "thehamkercat/telegram-spam-ham":
        ...
        true_labels = dataset['text_type']; eval_messages = dataset['text']
        print(f"✅ Loaded {len(eval_messages)} messages for evaluation.")
    except Exception as e:
        print(f"❌ ERROR: Failed to process HF dataset. Details: {e}"); return
        
    # --- 3. Run Hybrid Classification ---
    print(f"\nClassifying {len(eval_messages)} messages... This will take time.")
    results = []
    spam_idx = np.where(le.classes_ == 'spam')[0][0]
    start_time_total = time.perf_counter()
    for message in tqdm(eval_messages, desc="Classifying HF Dataset"):
        nb_probs = nb_pipeline.predict_proba([message])[0]; spam_prob = nb_probs[spam_idx]
        if spam_prob > 0.85: prediction = "spam"
        elif spam_prob < 0.15: prediction = "ham"
        else:
            q_emb = get_embeddings([message], "query", 1); _, indices = faiss_index.search(q_emb.astype('float32'), 5)
    ...
    print("\n" + "="*80); print(f"  FINAL REPORT: SpamGuard Hybrid on '{HF_DATASET_ID}'"); print("="*80)
    accuracy = accuracy_score(true_labels, results)
    avg_time_ms = (total_time_s * 1000) / len(true_labels)
    print(f"\nOverall Accuracy: {accuracy:.2%}"); print(f"Total Prediction Time: {total_time_s / 60:.2f} minutes"); print(f"Average Prediction Time: {avg_time_ms:.2f} ms/message")
    all_known_labels = ['ham', 'spam']
    print("\nClassification Report:"); print(classification_report(true_labels, results, labels=all_known_labels, zero_division=0))
    print("\nConfusion Matrix:"); cm = confusion_matrix(true_labels, results, labels=all_known_labels); print(cm)
    plt.figure(figsize=(8, 6)); sns.heatmap(cm, annot=True, fmt='d', cmap='magma', xticklabels=all_known_labels, yticklabels=all_known_labels); plt.title(f'SpamGuard Hybrid Performance\non {HF_DATASET_ID}'); plt.ylabel('True Label'); plt.xlabel('Predicted Label'); plt.show()

if __name__ == "__main__":
    evaluate_on_huggingface()