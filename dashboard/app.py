...
def set_config(mode: str, knn_dataset_file: str):
    """Sends the new configuration to the backend."""
    try:
        payload = {"mode": mode, "knn_dataset_file": knn_dataset_file}
        response = requests.post(f"{API_BASE_URL}/config", json=payload)
        response.raise_for_status()
        return response.json()
    except requests.exceptions.RequestException as e:
        st.error(f"API Error: Could not set config. {e}")
        return None

# --- Main Application Logic ---
st.title("🛡️ SpamGuard AI: An Adaptive Spam Filtering System")

# --- THE DEFINITIVE, NON-BLOCKING WARMUP & STATUS DISPLAY ---
status_response = check_backend_status()

# Check if the backend is ready ON THIS SCRIPT RUN
if not (status_response and status_response.get("is_ready")):
    # If NOT ready, display waiting message and schedule a rerun.
    st.info("⏳ SpamGuard AI engine is warming up... This may take several minutes on the first run.")
    st.warning("The backend is loading models in the background. This page will refresh automatically.")
    
    # Instruct Streamlit to wait and then rerun the script from the top.
    time.sleep(3) 
    st.rerun()

else:
    # If the backend IS ready, the script proceeds and draws the entire application.
    
    # --- Persistent Status Indicators ---
    config_response = get_config()
    col1, col2, col3 = st.columns(3)
    if config_response:
        col1.metric("Mode", config_response.get('mode', 'N/A').upper())
    ...
    if status_response and status_response.get("is_loading_new_config"):
        col3.warning(f"⏳ {status_response.get('loading_status_message', 'Applying new config...')}")
        time.sleep(3)
        st.rerun() # Poll for updates while a new config is loading
    else:
        col3.success("✅ System Ready")



    # --- All UI sections below are rendered only after the warmup is complete ---
    ...
    if st.session_state.last_classified_message:
        ...
        col1a.metric("Confidence", f"{conf:.2%}")
        col2a.metric("Model Used", model)
        if res.get('evidence'):
            with st.expander("💡 See Why (Explainable AI)"):
                st.write("Similar messages from the database:")
                ...
                if st.button("Activate Selected Model", type="primary", key="activate_selected_model_button"):
                    with st.spinner(f"Activating model '{model_to_activate}'..."):
                        ...
                        if response and response['status'] == 'success':
                            ...
                            st.info("The new model is now loading in the background. The UI will update when ready.")
    ...
    with tab_config:
        ...
        if config_response:
            ...
            selected_mode = st.radio("Choose classification mode:", options=mode_options, index=mode_options.index(config_response["mode"]), horizontal=True, key="classifier_mode_radio")
            ...
            if selected_mode == "hybrid" or selected_mode == "knn_only":
                ...
                selected_knn_dataset = st.selectbox("Select dataset for k-NN index:", options=available_datasets, index=default_dataset_index, key="knn_dataset_selector")
            else:
                selected_knn_dataset = config_response["knn_dataset_file"]
