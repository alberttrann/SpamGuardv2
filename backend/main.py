...
# --- Adjusted Data Generation Endpoint to handle new generator output ---
    ...
    async def event_stream():
        while True:
            ...
            # ... (other providers)
                ...
                generator = llm_generator.generate_with_lmstudio(
                    model=req.model,
                    label_to_generate=req.label_to_generate
                )
            elif req.provider == 'openrouter':
                if not req.api_key:
                    yield "data: Error: OpenRouter requires an API key.\n\n"
                    break
