...
HF_DATASET_ID = "SetFit/enron_spam"
DATASET_SPLIT = 'test'
...
def evaluate_on_enron():
    """
    Main function to evaluate the final SpamGuard Hybrid model on the Enron HF dataset.
    """
    print("\n" + "="*80)
    ...
    try:
        ...
        dataset = dataset.filter(lambda x: x['label_text'] is not None and x['message'] is not None and x['message'].strip() != "")
        
        # As you correctly identified, the labels are in 'label_text' and the content in 'message'
        true_labels = [label.lower() for label in dataset['label_text']]
        eval_messages = dataset['message']
        print(f"✅ Loaded {len(eval_messages)} messages from the '{DATASET_SPLIT}' split.")
    ...
    for message in tqdm(eval_messages, desc="Classifying Enron Dataset"):
