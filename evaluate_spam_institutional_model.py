# evaluate_spam_institute_model.py
...
# 1. Enter the path to the evaluation file you want to use.
EVALUATION_FILE_PATH = "evaluation_data.txt"
# EVALUATION_FILE_PATH = "only_tricky_ham_test_set.txt"
# EVALUATION_FILE_PATH = "evaluation_data.txt"

# 2. Set the batch size for inference.
BATCH_SIZE = 16
...
def load_test_data(file_path: str):
    """
    Loads and parses a .txt file with the format 'label,message'.
    """
    if not os.path.exists(file_path):
        print(f"❌ ERROR: Evaluation file not found at '{file_path}'. Please check the path.")
        return None, None
...
def evaluate_bert_classifier():
    """
    Main function to load the specified BERT model and run a full evaluation.
    """
    ...
    try:
        ...
        print(f"❌ ERROR: Could not load model from Hugging Face. Details: {e}")
        return
    ...
    with torch.no_grad(): # Disable gradient calculations for speed
        for i in tqdm(range(0, len(eval_messages), BATCH_SIZE), desc="Classifying Batches"):
            ...
            # Tokenize the batch and send to the correct device
            inputs = tokenizer(batch_messages, padding=True, truncation=True, return_tensors="pt", max_length=512)
            inputs = {k: v.to(device) for k, v in inputs.items()}
            
            # Get model outputs (logits)
            outputs = model(**inputs)
            
            # Get the index of the highest logit for each message in the batch
            predicted_indices = torch.argmax(outputs.logits, dim=1)
            
            end_time = time.perf_counter()
            total_time_s += (end_time - start_time)
            
            # Convert indices back to the model's native string labels (e.g., 'Spam')
            native_predictions = [model.config.id2label[idx.item()] for idx in predicted_indices]
            
            # Translate the native labels to our standard labels ('ham', 'spam') using the map
            translated_predictions = [LABEL_MAP.get(pred, "ham") for pred in native_predictions]
    ...
    print("\n" + "="*60)
    print(f"  DETAILED REPORT FOR: {MODEL_ID}")
    print(f"  ON TEST SET: {test_set_name}")
    print("="*60)
    ...
    # Ensure a 2x2 report even if one class is missing (e.g., in the tricky ham set)
    ...
    plt.title(f'BERT ({os.path.basename(MODEL_ID)})\n on {test_set_name}')
...
if __name__ == "__main__":
    if not EVALUATION_FILE_PATH:
        print("❌ ERROR: Please configure the EVALUATION_FILE_PATH variable at the top of the script.")
    else:
